...
bool QLottieVectorImagePluginGenerator::generate(QQuickGenerator *generator)
{
    ...
    if (!jsonSource.isEmpty()) {
        if (root.parseSource(jsonSource, QUrl::fromLocalFile(source.fileName())) >= 0) {
            ...
            QQuickQmlGenerator *qmlGenerator = static_cast<QQuickQmlGenerator *>(generator);
            qmlGenerator->addExtraImport(QStringLiteral("Qt.labs.lottieqt.VectorImageHelpers"));
            qmlGenerator->setGeneratorFlags(
                qmlGenerator->generatorFlags().setFlag(QQuickVectorImageGenerator::TimelineAnimation));
